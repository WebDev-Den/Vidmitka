...
it.skipIf(!process.env.VIDMITKA_ATTENDANCE_TEST_SCHEMA)(
  ...
  async () => {
    ...
    const updated = await updateClassPeriodAction(first.id, initialPeriodActionState, periodForm(first, " #48c5b5 "));
    expect(updated.success).toBe(true);
    expect((await listClassPeriods()).find((period) => period.id === first.id)).toEqual({ ...first, color: "#48C5B5" });
    const timeline = getDayTimeline(await listClassPeriods(), new Date("2026-08-28T05:30:00Z"));
    expect(timeline.currentSegment).toMatchObject({ kind: "period", number: first.number, color: "#48C5B5" });

    const newPeriod = { number: 9, startTime: "21:00", endTime: "21:40" };
    expect((await createClassPeriodAction(initialPeriodActionState, periodForm(newPeriod, "#243B3A"))).success).toBe(true);
    const ninth = (await listClassPeriods()).find((period) => period.number === 9);
    expect(ninth).toMatchObject({ ...newPeriod, color: "#243B3A", isActive: true });
    ...
    expect((await updateClassPeriodAction(ninth.id, initialPeriodActionState, periodForm(ninth, "#48C5B5"))).success).toBe(true);
    expect((await listClassPeriods()).find((period) => period.id === ninth.id)?.color).toBe("#48C5B5");
    ...
    for (const color of ["", "purple", "#A855F7", "url(https://example.test)"]) {
    ...
    expect((await listClassPeriods()).find((period) => period.id === first.id)?.color).toBe("#48C5B5");

    expect((await updateClassPeriodAction(first.id, initialPeriodActionState, periodForm(first, "#48C5B5", "deactivate"))).success).toBe(true);
    const inactive = (await listClassPeriods()).find((period) => period.id === first.id);
    expect(inactive).toMatchObject({ isActive: false, color: "#48C5B5" });
    ...
    expect((await updateClassPeriodAction(first.id, initialPeriodActionState, periodForm(first, "#48C5B5", "activate"))).success).toBe(true);
    ...
    expect((await listClassPeriods()).find((period) => period.id === first.id)).toMatchObject({ isActive: true, color: "#48C5B5" });
    expect(await listClassPeriods()).toHaveLength(original.length + 1);
  },
  120000,
);
