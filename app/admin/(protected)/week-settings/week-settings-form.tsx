...
export function WeekSettingsForm({ settings }: { settings: ScheduleWeekConfiguration | null }) {
  ...
  return <form action={action} className="lesson-editor">
    <div className="settings-form-heading"><h2>Чергування чисельника й знаменника</h2><p>Оберіть будь-яку відому дату та вкажіть, чи це чисельник або знаменник. Межа тижня — понеділок; далі тип автоматично змінюється кожні сім днів.</p></div>
    ...
    <label>Тип базового тижня<select name="anchorWeekType" defaultValue={settings?.anchorWeekType ?? "numerator"} disabled={pending}>
      <option value="numerator">Чисельник</option><option value="denominator">Знаменник</option></select></label>
