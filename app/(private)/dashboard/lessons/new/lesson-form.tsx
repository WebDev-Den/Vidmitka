...
export function LessonForm({ subjects, rooms, periods, lessonTypes, groups, students, teachers, isAdministrator, currentUserId }: {
  ...
  const directories = useLessonDirectories({ subject: subjects, room: rooms, lessonType: lessonTypes });
  ...
  return <form action={action} className="lesson-editor lesson-create-form compact-form"
    ...
    <LessonStudentPicker groups={groups} students={students} disabled={pending} optional />
    <button className="button button-primary" type="submit" disabled={pending || directories.isCreating || !ready}>{pending ? "Створення…" : "Створити заняття"}</button>
