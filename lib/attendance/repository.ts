...
import { getScheduleDayContext } from "@/lib/schedule-calendar/repository";
...
export async function listJournalLessons(teacherUserId: string, date: string) {
  if (!isJournalDate(date)) return { lessons: [], weekType: null, day: null };
  const day = await getScheduleDayContext(date);
  const weekType = day.weekType;
  const sql = getDb();
  const rows = await sql`
    WITH scheduled AS (
      ...
      WHERE l.teacher_user_id = ${teacherUserId} AND ts.teacher_user_id = ${teacherUserId}
        AND l.day_of_week = ${day.dayOfWeek}
  ...
  return { weekType, day, lessons: rows.map((row): JournalLesson => ({
...
export async function saveAttendance(teacherUserId: string, input: {
  date: string; key: string; version: number; calendarToken: string; marks: { studentId: string; status: string }[];
}) {
  ...
  const { lessons, day } = await listJournalLessons(teacherUserId, input.date);
  if (!day || input.calendarToken !== day.token) return fail("Календар розкладу змінився. Оновіть журнал перед збереженням.");
  ...
  const sql = getDb();
  // Спільний lock дати з календарем. Окремий наступний statement у Read Committed
  // бачить завершені зміни після очікування lock, а не попередній snapshot.
  const [, savedRows] = await sql.transaction([
    sql`SELECT lock_schedule_day(${input.date}::DATE)`,
    sql`
    WITH current_day AS MATERIALIZED (
      SELECT * FROM get_schedule_day(${input.date}::DATE)
    ), imported AS (
      ...
      SELECT NOT EXISTS (SELECT student_id FROM current_roster EXCEPT SELECT "studentId" FROM imported)
        AND NOT EXISTS (SELECT "studentId" FROM imported EXCEPT SELECT student_id FROM current_roster)
        AND EXISTS (SELECT 1 FROM current_day WHERE context_token = ${input.calendarToken}) AS ok
    ), created AS (
      INSERT INTO attendance_sessions (lesson_id, teacher_subject_id, teacher_user_id, held_on,
        subject_name, room_name, period_number, start_minute, end_minute, lesson_type_name)
      SELECT l.id, l.teacher_subject_id, ${teacherUserId}, ${input.date}::DATE,
        ${lesson.subjectName}, ${lesson.roomName}, ${lesson.periodNumber}, ${lesson.startMinute}, ${lesson.endMinute}, t.name
      FROM lessons l LEFT JOIN lesson_types t ON t.id = l.lesson_type_id CROSS JOIN valid CROSS JOIN current_day
      WHERE ${input.version} = 0 AND valid.ok AND l.id = ${lesson.lessonId}::BIGINT
        ...
        AND l.day_of_week = current_day.schedule_day
        AND (l.week_type = 'both' OR l.week_type = current_day.week_type)
  ...
  const result = savedRows as unknown as { student_id: string | number }[];
