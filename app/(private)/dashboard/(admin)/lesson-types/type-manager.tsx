...
function TypeRow({ type }: { type: LessonType }) {
  ...
  return <tbody>
    <tr>
      ...
      <td><ColorField key={type.color} color={type.color} form={formId}
        label={"Колір типу: " + type.name} hideLabel disabled={pending} /></td>
      ...
      <td className="management-actions-cell"><div className="management-actions">
        <form id={formId} action={action}>
          ...
          <button className="button button-light" disabled={pending} aria-label={"Зберегти назву й колір: " + type.name}>
    ...
    <ManagementFeedback state={toggleState} colSpan={4} />
  </tbody>;
}
...
export function LessonTypeManager({ types }: { types: LessonType[] }) {
  ...
  return <ManagementTable caption="Типи занять" columns={["Назва типу", "Колір", "Стан", "Дії"]} minWidth={740}>
    <tbody>
      <tr className="management-new-row">
        ...
        <td><ColorField form={formId} label="Колір нового типу заняття" hideLabel disabled={pending} /></td>
    ...
    {!types.length && <tbody><tr><td colSpan={4} className="management-muted">Типів занять ще немає. Додайте перший тип.</td></tr></tbody>}
  </ManagementTable>;
}
