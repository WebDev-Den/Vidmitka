...
export type LessonDraft = {
  subjectId: string; roomId: string; classPeriodId: string; dayOfWeek: number;
...
export function validateLessonDraft(input: LessonInput): { ok: true; value: LessonDraft } | { ok: false; message: string } {
  ...
  return { ok: true, value: { subjectId: input.subjectId, roomId: input.roomId, classPeriodId: input.classPeriodId,
